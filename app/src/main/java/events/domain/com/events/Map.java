...
public class Map extends AppCompatActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener  {
                ...
                LocationServices.GeofencingApi.addGeofences(googleApiClient, geofencingRequest, pendingIntent).setResultCallback(new ResultCallback<Status>() {
                    ...
                    public void onResult(@NonNull Status status) {
                        if (status.isSuccess()) {
                            ...
                            showNotification();
                            Toast.makeText(Map.this, "Nearby events at JKUAT", Toast.LENGTH_LONG).show();
                        } else {
    ...
    protected void onResume() {
        super.onResume();
        int response = GoogleApiAvailability.getInstance().isGooglePlayServicesAvailable(Map.this);
        if (response != ConnectionResult.SUCCESS) {
            Log.d(TAG, "Google Play Service Not Available");
            GoogleApiAvailability.getInstance().getErrorDialog(Map.this, response, 1).show();
        } else {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        LatLng latLng = Constants.AREA_LANDMARKS.get(Constants.GEOFENCE_ID_STAN_UNI);
        googleMap.addMarker(new MarkerOptions().position(latLng).title("Jkuat University"));
        ...
        LatLng library = new LatLng(-1.09694,37.01535409999997);
        googleMap.addMarker(new MarkerOptions().position(library).title("Library").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_ORANGE)));

        LatLng engineering = new LatLng(-1.0952202,37.01204419999999);
        googleMap.addMarker(new MarkerOptions().position(engineering).title("Engineering").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_ORANGE)));

        LatLng agriculture = new LatLng(-1.0945159,37.0138025);
        googleMap.addMarker(new MarkerOptions().position(agriculture).title("Agriculture").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_ORANGE)));
    }
    ...
    public void showNotification() {
        ...
        Notification notification = new NotificationCompat.Builder(this)
                .setTicker("JKUATEvents")
                .setSmallIcon(R.drawable.logo)
                .setContentTitle("JKUAT nearby events")
                .setContentText("Click to view more events")
                .setContentIntent(pi)
                .setAutoCancel(true)
